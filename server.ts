    ...
    if (provider === "gemini") {
      ...
      if (gKey && gKey !== "MY_GEMINI_API_KEY") {
        ...
        for (const modelName of geminiModels) {
          try {
            console.log(`[AI Fallback] Attempting Gemini fallback model: ${modelName}`);
            ...
            console.warn(`[AI Fallback] Gemini [${modelName}] trial failed: ${errStr}`);
            errors.push(`Gemini [${modelName}]: ${errStr}`);
          }
        }
      }
...
${req.body.explanation || "No offline rationale registered."}`
  ...
  app.post("/api/ai-classify-subjects", async (req, res) => {
    try {
      ...
      const chunkSize = 15;
      const classifications: { id: string; subject: string }[] = [];
      ...
      for (let i = 0; i < questions.length; i += chunkSize) {
        ...
        try {
          ...
          Object.entries(parsed).forEach(([id, subject]) => {
            classifications.push({ id, subject: String(subject) });
          });
        } catch (parseErr) {
          console.error("Failed to parse classification chunk:", parseErr, responseText);
          chunk.forEach((q) => {
            classifications.push({ id: q.id, subject: "General Knowledge" });
          });
        }
      }
