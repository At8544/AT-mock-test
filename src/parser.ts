...
export async function parseHtmlToQuestions(htmlContent: string, targetExam: string): Promise<Question[]> {
  ...
  let questions = await tryExtractQuestionsFromScripts(htmlContent, targetExam);
  if (questions.length >= 1) {
    console.log(`Parsed ${questions.length} questions using Strategy 0 (Script variables & fetch fallback)`);
    return questions;
  }
